 ...
 * @author ViiSE
 */
@Component("clientListRequestProcessorAdmin")
@Scope("prototype")
public class ClientListRequestProcessor implements ProcessorAdmin {
    ...
    public CommandResult apply(AdminCommand admCommand) {
        if(!checker.isNull(admCommand.name())) {
            return commandResult(admCommand.name(), LogMessageTypeEnum.INFO, ResultTypeMessageEnum.TRUE,
                    clients, admCommand.name() + " requested clients list");
        }
        else
