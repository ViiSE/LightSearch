 ...
 * @author ViiSE
 */
@Component("cancelSoftCheckProcessorClient")
@Scope("prototype")
public class CancelSoftCheckProcessor implements ProcessorClient {
    ...
    public CancelSoftCheckProcessor(LightSearchServerDTO serverDTO, LightSearchChecker checker, ClientDAO clientDAO,
    ...
    public CommandResult apply(ClientCommand clientCommand) {
        if(!checker.isNull(clientCommand.IMEI(), clientCommand.userIdentifier(),
            ...
            if(!blacklist.contains(clientCommand.IMEI())) {
                try {
                    DatabaseCommandMessage dbCmdMessage = dbCmdMsgProducer.getDatabaseCommandMessageCancelSoftCheckDefaultWindowsJSONInstance(
                            clientCommand.command(), clientCommand.IMEI(), clientCommand.userIdentifier(), clientCommand.cardCode());
