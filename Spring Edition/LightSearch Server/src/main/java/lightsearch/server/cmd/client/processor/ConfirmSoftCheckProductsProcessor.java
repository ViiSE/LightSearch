 ...
 * @author ViiSE
 */
@Component("confirmSoftCheckProductsProcessorClient")
@Scope("prototype")
public class ConfirmSoftCheckProductsProcessor implements ProcessorClient {
    ...
    public CommandResult apply(ClientCommand clientCommand) {
        if(!checker.isNull(clientCommand.IMEI(), clientCommand.userIdentifier(),
            ...
            if(!blacklist.contains(clientCommand.IMEI())) {
                try {
                    DatabaseCommandMessage dbCmdMessage = dbCmdMsgProducer.getDatabaseCommandMessageConfirmSoftCheckProductsDefaultWindowsJSONInstance(
