 ...
 * @author ViiSE
 */
@Component("openSoftCheckProcessorClient")
@Scope("prototype")
public class OpenSoftCheckProcessor implements ProcessorClient {
    ...
    public OpenSoftCheckProcessor(LightSearchServerDTO serverDTO, LightSearchChecker checker, ClientDAO clientDAO,
    ...
    public CommandResult apply(ClientCommand clientCommand) {
        if(!checker.isNull(clientCommand.IMEI(), clientCommand.userIdentifier(), clientCommand.cardCode())) {
            if(!blacklist.contains(clientCommand.IMEI())) {
                try {
                    DatabaseCommandMessage dbCmdMessage = dbCmdMsgProducer.getDatabaseCommandMessageOpenSoftCheckDefaultWindowsJSONInstance(
