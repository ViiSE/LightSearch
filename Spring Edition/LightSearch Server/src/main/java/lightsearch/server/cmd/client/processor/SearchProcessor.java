 ...
 * @author ViiSE
 */
@Component("searchProcessorClient")
@Scope("prototype")
public class SearchProcessor implements ProcessorClient {
    ...
    public SearchProcessor(LightSearchServerDTO serverDTO, LightSearchChecker checker, ClientDAO clientDAO,
    ...
    public CommandResult apply(ClientCommand clientCommand) {
        if(!checker.isNull(clientCommand.IMEI(), clientCommand.barcode(),
            ...
            if(!blacklist.contains(clientCommand.IMEI())) {
                try {
                    DatabaseCommandMessage dbCmdMessage = dbCmdMsgProducer.getDatabaseCommandMessageSearchDefaultWindowsJSONInstance(
