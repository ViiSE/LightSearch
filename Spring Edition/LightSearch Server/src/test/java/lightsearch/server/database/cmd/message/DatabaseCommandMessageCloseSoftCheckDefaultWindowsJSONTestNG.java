...
public class DatabaseCommandMessageCloseSoftCheckDefaultWindowsJSONTestNG extends AbstractTestNGSpringContextTests {
    ...
    public void setUpClass() {
        ...
        cmdDTO.setCommand("close_soft_check");
        cmdDTO.setIMEI("111111111111111");
        cmdDTO.setUserIdentifier("777");
        cmdDTO.setCardCode("22505");
        cmdDTO.setDelivery("1");

        dbCmdMsg = dbCmdMsgProducer.getDatabaseCommandMessageCloseSoftCheckDefaultWindowsJSONInstance(
                cmdProducer.getClientCommandDefaultInstance(cmdDTO));
    }

    @Test
    public void message() {
        testBegin("DatabaseCommandMessageCloseSoftCheckDefaultWindowsJSON", "message()");

        String message = dbCmdMsg.message();
        assertNotNull(message, "Message is null!");
        assertFalse(message.isEmpty(), "Message is empty!");
        System.out.println("Close SoftCheck message: " + message);

        testEnd("DatabaseCommandMessageCloseSoftCheckDefaultWindowsJSON", "message()");
    }
}
