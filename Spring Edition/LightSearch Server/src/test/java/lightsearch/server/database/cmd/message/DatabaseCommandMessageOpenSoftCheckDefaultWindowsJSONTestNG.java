...
public class DatabaseCommandMessageOpenSoftCheckDefaultWindowsJSONTestNG extends AbstractTestNGSpringContextTests {
    ...
    public void setUpClass() {
        ...
        cmdDTO.setCommand("open_soft_check");
        cmdDTO.setIMEI("111111111111111");
        cmdDTO.setUserIdentifier("777");
        cmdDTO.setCardCode("22505");

        dbCmdMsg = dbCmdMsgProducer.getDatabaseCommandMessageOpenSoftCheckDefaultWindowsJSONInstance(
                cmdProducer.getClientCommandDefaultInstance(cmdDTO));
    }

    @Test
    public void message() {
        testBegin("DatabaseCommandMessageOpenSoftCheckDefaultWindowsJSON", "message()");

        String message = dbCmdMsg.message();
        assertNotNull(message, "Message is null!");
        assertFalse(message.isEmpty(), "Message is empty!");
        System.out.println("Open SoftCheck message: " + message);

        testEnd("DatabaseCommandMessageOpenSoftCheckDefaultWindowsJSON", "message()");
    }
}
